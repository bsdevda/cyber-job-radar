...
def build_report_payload(
    ...
    summary = {
        ...
        "sources_failed": sum(not details.get("ok") for details in source_status.values()),
...
def render_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        f"**Date:** {date}",
        f"**Jobs collected:** {summary['jobs_collected']}",
    ...
    for source, details in payload["source_status"].items():
        ...
        suffix = f" - {details.get('error')}" if details.get("error") else ""
        lines.append(f"- **{source.title()}:** {status} ({details.get('jobs', 0)} jobs){suffix}")
    lines.extend(
        [
            "",
            "## ChatGPT Handoff",
